...
'         For j = 0 To 7
'             llNum1 = llNum And 1
'             llNum = (llNum - llNum1) / 2
'             if llNum < 0 Then llNum = llNum - &H80000000
'             If llNum1 Then llNum = llNum Xor &HEDB88320
'         Next
'         vTable(i) = llNum
'     Next
' End Sub
'
' 'CRC32計算用テーブル設定用文字列をファイルに保存する
' Sub SaveCRC32Table()
'     Set loFs = CreateObject("Scripting.FileSystemObject")
'     lsPath = loFs.GetParentFolderName(WScript.ScriptFullName) & "\CRC32_Table.txt"
'     Set loTs = loFs.OpenTextFile(lsPath,2,True)
'     loTs.WriteLine "vTable = Array( _"
'     lsStr = vbTab
'     For i = 0 To UBound(vTable)
'         lsStr = lsStr & "&H" & Right("0000000" & Hex(vTable(i)),8)
'         If i < UBound(vTable) Then
'             lsStr = lsStr & ","
'         Else
'             lsStr = lsStr & " "
'         End If
...
vTable = Array( _
    ...
    &HB3667A2E,&HC4614AB8,&H5D681B02,&H2A6F2B94,&HB40BBE37,&HC30C8EA1,&H5A05DF1B,&H2D02EF8D  _
)

' ファイルからCRC32を計算
Public Function GetCrc32FromFile(lsPath)
    Set loStr = CreateObject("ADODB.Stream")
    loStr.Open
    loStr.Type = 1
    loStr.LoadFromFile(lsPath)
    llNum = &HFFFFFFFF
    For i = 1 To loStr.Size
        lbByte = CLng(AscB(loStr.Read(1)))
        llNum = (Int(llNum / 256) And &HFFFFFF) Xor vTable((llNum Xor lbByte) And &HFF)
    Next
    loStr.Close
    Set loStr = nothing
    GetCrc32FromFile = Not llNum
End Function

' メイン処理
Sub Main()
    For i = 0 To WScript.Arguments.Count - 1
        WScript.Echo Hex(GetCrc32FromFile(WScript.Arguments(i))) & vbTab & WScript.Arguments(i)
    Next
End Sub
